...
cosine_comparison = pd.DataFrame(
    {"First_Work": [], "Second_Work": [], "Cosine_sim": []}
)
        ...
        new_obs = pd.DataFrame(
            {
                "First_Work": [i],
                "Second_Work": [j],
                "Cosine_sim": [round(cos_sim[0, 0], 3)],
            }
        )
        cosine_comparison = pd.concat([cosine_comparison, new_obs], ignore_index=True)


cosine_matrix = cosine_comparison.pivot(
    index="First_Work", columns="Second_Work", values="Cosine_sim"
)
...
cosine_comparison_total = pd.DataFrame(
    {"First_Work": [], "Second_Work": [], "Cosine_sim": []}
)
for i in df_freq_periods.drop(columns=["word"]).columns:
    for j in df_freq_periods.drop(columns=["word"]).columns:
        ...
        cos_sim = cosine_similarity(
            np.array([df_freq_periods[i]]), np.array([df_freq_periods[j]])
        )
        new_obs = pd.DataFrame(
            {
                "First_Work": [i],
        ...
        cosine_comparison_total = pd.concat(
            [cosine_comparison_total, new_obs], ignore_index=True
        )


cosine_comparison_total = cosine_comparison_total.pivot(
    index="First_Work", columns="Second_Work", values="Cosine_sim"
)

cosine_comparison_total.to_csv(
    os.path.join("cosine_sim", "cosine_comparison_total.csv")
)
