...
for i in df_freq.drop(columns=["word"]).columns:
    for j in df_freq.drop(columns=["word"]).columns:
        ...
        new_obs = pd.DataFrame(
            {"First_Work": [i], "Second_Work": [j], "euc_sim": [round(euc_sim, 3)]}
        )
        euc_comparison = pd.concat([euc_comparison, new_obs], ignore_index=True)


euc_matrix = euc_comparison.pivot(
    index="First_Work", columns="Second_Work", values="euc_sim"
)
...
euc_comparison_total = pd.DataFrame(
    {"First_Work": [], "Second_Work": [], "euc_sim": []}
)
for i in df_freq_periods.drop(columns=["word"]).columns:
    for j in df_freq_periods.drop(columns=["word"]).columns:

        euc_sim = distance.euclidean(
            np.array([df_freq_periods[i]]), np.array([df_freq_periods[j]])
        )
        new_obs = pd.DataFrame(
            {"First_Work": [i], "Second_Work": [j], "euc_sim": [round(euc_sim, 3)]}
        )
        euc_comparison_total = pd.concat(
            [euc_comparison_total, new_obs], ignore_index=True
        )


euc_comparison_total = euc_comparison_total.pivot(
    index="First_Work", columns="Second_Work", values="euc_sim"
)
